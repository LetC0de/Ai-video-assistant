...
class ConversationOutSchema(BaseModel):
    ...
    The DB column is `id`, but the client contract uses `conversation_id`. We
    read the value from `id` via `validation_alias` so it can be populated from
    the ORM row, while `conversation_id` remains the *serialisation* key — so
    FastAPI emits `{"conversation_id": 101, ...}` rather than `{"id": 101, ...}`.
    """
...
class ConversationMessagesSchema(BaseModel):
    """Response for GET /conversations/{id}/messages (history API)."""
