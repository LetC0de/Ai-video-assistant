...
async def create_new_conversation(
    ...
    The frontend calls this on the first message of a chat. Returns
    conversation_id, which is then sent with every /chat/query request so the
    LangGraph thread_id is deterministic.
    """
...
async def get_conversation(
    ...
    """Fetch a single conversation — used when switching or restoring one after
    a page reload. Validates ownership."""
...
async def get_conversation_message_history(
    ...
    """Return a conversation's message history for the UI.
