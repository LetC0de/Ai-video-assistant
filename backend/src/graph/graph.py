...
Flow:
    START
      │
      ▼
    retrieve_documents  → Qdrant (filtered by meeting_id)
      │
      ▼
...
Streaming note: `generate_answer` uses `llm.ainvoke` (full response) so the
AIMessage gets checkpointed. Token-by-token streaming happens in `streaming.py`,
which calls Mistral directly using the same prompt + context. That keeps
checkpointing + streaming from doubling up on the same content.
"""
...
def get_compiled_graph():
    ...
    `lru_cache` is a belt-and-suspenders guard against accidental double-init.
    The checkpointer module is the real source of truth — it must be
    initialised (async) during the FastAPI lifespan before the first request.
    """
